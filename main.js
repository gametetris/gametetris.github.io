...
function startGame() {
  ...
  tetrisContainer.classList.remove('hide-tetris')
  tetrisContainer.classList.add('show-tetris')
}
...
// Variabel untuk menyimpan balok cadangan
let heldTetromino = null;
let canHold = true;

// Fungsi untuk menukar balok
function holdTetromino() {
  if (!canHold) return;

  eraseTetromino();
  if (heldTetromino) {
    // Jika ada balok dalam cadangan, tukar
    const temp = heldTetromino;
    heldTetromino = currentTetromino;
    currentTetromino = { ...temp, row: 0, col: Math.floor((BOARD_WIDTH - temp.shape[0].length) / 2) };
  } else {
    // Jika tidak ada, masukkan balok saat ini ke cadangan dan ambil balok baru
    heldTetromino = currentTetromino;
    currentTetromino = randomTetromino();
  }
  canHold = false; // Hanya bisa menukar sekali per balok
  drawTetromino();
  moveGhostTetromino();
}

// Tambahkan event listener untuk tombol Z
...
function lockTetromino() {
  ...
  // Reset kemampuan untuk menukar balok
  canHold = true;
  // Current tetromino
  currentTetromino = randomTetromino();
}

let currentZ = "Z"; // Huruf Z awal

// Fungsi untuk mengganti dan memperbarui huruf Z
function updateZ(newChar) {
  currentZ = newChar;
  document.getElementById("z-display").textContent = currentZ;
}
